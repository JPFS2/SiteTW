...

{
...

    {
        ...
        while ($cliente = $results->fetchObject()) {
            ...
            $tr .= View::render('admin/manutencao/clientes',
...

    {
        ...
        while ($produto = $results->fetchObject()) {
            ...
            $tr .= View::render('admin/manutencao/produto_manutencao',
...

    {
        ...
        while ($produto = $results->fetchObject()) {
            ...
            $tr .= View::render('admin/manutencao/produto_insert',
...

    {
        ...
        $results = M_Manutencao::buscarJoinPag('dtencerrado is null and tipo = '. '"Garrantia"', null, null, 'manutencao.*, manutencaopagamentos.moeda,  manutencaopagamentos.valor');
        ...
        while ($produto = $results->fetchObject()) {
            ...
            $tr .= View::render('admin/manutencao/produto',
    ...
    public static function addTroca($request){
        ...
        $obProduto->fornec = $postVars['cliente'];
    ...
    public static function removeManutencao($id)

    {

        $obCarrinho = M_Manutencao::buscar('codmanutencao = '.$id)->fetchObject(M_Manutencao::class);



        if ($obCarrinho instanceof M_Manutencao) {

            $produto = M_Produto::buscar('codprod = '.$obCarrinho->entrada)->fetchObject(M_Produto::class);



            if($obCarrinho->custo > 0){

                $obCarrinho->custo = $obCarrinho->custo * -1;

                if($produto->codsecao == 1){
                    $produto->aumentaCusto($obCarrinho->custo);
                }
        ...
        header("Location: https://juninhodoiphone.com/controle/admin/manutencao");

        die();

    }

    public static function finalizaGarantia($request)

    {



        M_Manutencao::finalizarGarantia();



        header("Location: https://juninhodoiphone.com/controle/admin/garantia");
...

    {
        ...
        $manutenção->custo = $postVars['custo'];
        ...
        if(isset($postVars['itensUtilizados'])){
            ...
            foreach ($postVars['itensUtilizados'] as $produto){
                ...
                $manutenção->custo += M_Produto::buscar('codprod = '.$produto,null,null,'pcompra')->fetchObject()->pcompra;
        ...
        if(isset($postVars['garantia'])){
            ...
            if(isset($postVars['manutencao'])){
                ...
                if($produto->codsecao == 1){
                    $produto->aumentaCusto(floatval($manutenção->custo));
                }
                ...
                $produtoDevolvido = explode('-',$postVars['saida']);
                ...
                if (isset($postVars['perca'])){

                    $perca = new M_Perca();

                    $perca->codprod = $manutenção->entrada;

                    $perca->quantidade = 1;



                    $custo = M_Produto::buscar('codprod = '.$perca->codprod,null,null,'pcompra')->fetchObject()->pcompra;

                    $perca->valor = $custo;



                    M_Produto::baixaEstoque($perca->codprod,$perca->quantidade);
