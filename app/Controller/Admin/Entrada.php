...
{    /*    * Metodo resposave por retornar o conteudo da view do home    *  @retunr string     */
    ...
    public static function getProdutoCombo(){
            ...
            $tr = '';
            $entradas = M_Entrada::buscarComProduto('cadprod.descricao <> ""');
            ...
            while($entrada = $entradas->fetchObject()){
                ...
                $tr .= View::render('admin/entrada/pedidos', [
                    'codcombo' => $entrada->codentrada,
                    'produtos' =>  $prodtos,
                    'quantidade' => $entrada->qt,
                    'custo' => $entrada->valor,
                    'custoT' => ($entrada->qt * $entrada->valor)
                ]);
            }

            $content = View::render('admin/entrada', [
                'tr' => $tr,
    ...
    public static function removeEntrada($codigo){
        ...
        $entrada = M_entrada::buscar("codentrada = ".$codigo)->fetchObject();

        
        M_Produto::retornaEstoque($entrada->codprod,($entrada->qt * -1));
        M_Entrada::excluir($codigo);


        header("Location: http://localhost/controle/admin/entrada");
        die();
    ...
    public static function addEntrada($request){
        ...
        $entrada = new M_Entrada();
        $entrada->codprod = array_shift($produto);
        $entrada->quantidade = $postVars['quantidade'];
        $entrada->valor =  $postVars['custo'];
        $entrada->valor = $entrada->valor/$entrada->quantidade;

        $entrada->cadastra();

        $produto = M_Produto::buscar('codprod = '.$entrada->codprod,null,null,'*')->fetchObject();

        $qt =  $entrada->quantidade +  $produto->qtestoque;
        $custoGeral = ($entrada->quantidade * $entrada->valor) + ($produto->qtestoque * $produto->pcompra);
        $novoCusto = $custoGeral / $qt;

        M_Produto::retornaEstoque($entrada->codprod,$entrada->quantidade);
        M_Produto::atualizaCompra($entrada->codprod, $novoCusto);
...

    {
        ...
        $tr = '';
        $results = M_Produto::buscar('codsecao <> 1 AND descricao <> ""', null, null, '*');
