...
class LucratividadePedido extends Page {
    ...
    */
    ...
    public static function getPedidos(){
        ...
        if($_SESSION ['admin'] ['usuario']['id'] == 999){

            $tr = '';

            $pedidos = M_Pedido::buscar('numped = 3450','pedido.numped desc',null,'pedido.* , cadcliente.cliente');

            while($pedido = $pedidos->fetchObject()){

                $credito = M_Credito::buscar('numpedBaixa = '.$pedido->numped)->fetchObject(M_Credito::class);



                $vlCredito = 0;

                if($credito instanceof M_Credito){
                    $vlCredito = $credito->vlcredito;
                }

                $vlcusto = M_PedidoItens::buscarItens('pedidoitens.numped = '.$pedido->numped,null,null,'(SUM(cadprod.pcompra + cadprod.pcusto) * pedidoitens.qt) as vlcusto')->fetchObject()->vlcusto;

                
                $vlliquido = $pedido->vltotal - $pedido->vldesconto + $pedido->acrescimo;
                $tipo = M_Credito::buscar('numpedBaixa = '.$pedido->numped)->fetchObject();
                $tpPedido = isset($tipo->numpedBaixa) ? '<span class="badge bg-warning">Troca</span>' : '<span class="badge bg-success">Venda</span>';

                $vlPromissoria = M_PedidoPagamento::buscar('moeda = "Promissoria" and numped = '.$pedido->numped ,null,null,'SUM(valor) as vltotal')->fetchObject()->vltotal;

                $date = date_create($pedido->dtpedido);

                $tr .= View::render('admin/lucropedidos/pedidos', [
                    'numped' => $pedido->numped,
                    'cliente' =>  $pedido->cliente,
                    'dtpedido' => date_format($date,'d/m/Y'),
                    'vlcusto' => $vlcusto,
                    'vlliq' => $vlliquido,
                    'vlpromissoria' => $vlPromissoria,
                    'vllucro' => $vlliquido - $vlcusto - $vlCredito - $vlPromissoria,
                    'vllucroR' => $vlliquido - $vlcusto - $vlCredito,
                    'vlcredito' => $vlCredito,
                    'percLucro' => $vlliquido - $vlcusto,
                    'tipo' => $tpPedido,
                    'perLucro' => ($vlliquido > 0) ? round((($vlliquido - $vlcusto - $vlCredito- $vlPromissoria) / $vlliquido) * 100,2) .'%' : 0,
            ]);
        ...
        $content = View::render('admin/lucropedidos', [
