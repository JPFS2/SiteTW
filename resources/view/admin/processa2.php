...
if($arquivo['type'] == "text/csv"){
    ...
    while($linha = fgetcsv($dados_arquivo, 1000, ";")){
        ...
        $query_usuario = "INSERT INTO cadprod (descricao, codsecao, pcompra, punit, qtestoque, modelo, fornec, dtimport) VALUES (:descricao, :codsecao, :pcompra, :punit, :qtestoque, :modelo, :fornec, :dtimport)";
        ...
        $cad_usuario->bindValue(':codsecao', ($linha[1] ?? "NULL"));
        $cad_usuario->bindValue(':pcompra', ($linha[2] ?? "NULL"));
        $cad_usuario->bindValue(':punit', ($linha[3] ?? "NULL"));
        $cad_usuario->bindValue(':qtestoque', ($linha[4] ?? "NULL"));
        $cad_usuario->bindValue(':modelo', ($linha[5] ?? "NULL"));
        $cad_usuario->bindValue(':fornec', ($linha[6] ?? "NULL"));
        $cad_usuario->bindValue(':dtimport', ($linha[7] ?? "NULL"));
