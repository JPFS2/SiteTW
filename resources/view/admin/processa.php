...
if($arquivo['type'] == "text/csv"){
    ...
    while($linha = fgetcsv($dados_arquivo, 1000, ";")){
        ...
        $query_usuario = "INSERT INTO cadprod (descricao, ean13, und, codsecao, peso, pcompra, punit, qtestoque, modelo, fornec, dtimport) VALUES (:descricao, :ean13, :und, :codsecao, :peso, :pcompra, :punit, :qtestoque, :modelo, :fornec, :dtimport)";
        ...
        $cad_usuario->bindValue(':ean13', ($linha[1] ?? "NULL"));
        $cad_usuario->bindValue(':und', ($linha[2] ?? "NULL"));
        $cad_usuario->bindValue(':codsecao', ($linha[3] ?? "NULL"));
        $cad_usuario->bindValue(':peso', ($linha[4] ?? "NULL"));
        $cad_usuario->bindValue(':pcompra', ($linha[5] ?? "NULL"));
        $cad_usuario->bindValue(':punit', ($linha[6] ?? "NULL"));
        $cad_usuario->bindValue(':qtestoque', ($linha[7] ?? "1"));
        $cad_usuario->bindValue(':modelo', ($linha[8] ?? "NULL"));
        $cad_usuario->bindValue(':fornec', ($linha[9] ?? "NULL"));
        $cad_usuario->bindValue(':dtimport', ($linha[10] ?? "NULL"));
